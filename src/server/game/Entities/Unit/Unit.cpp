...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    {
        // mechanical health drain
        if (spellProto)
        {
            int32 drainPct = 0;
            AuraEffectList const& a = GetAuraEffectsByType(SPELL_AURA_DRAIN_HEALTH_FROM_MECHANIC_DAMAGE);
            for (AuraEffectList::const_iterator i = a.begin(); i != a.end(); ++i)
            {
                if (spellProto->GetAllEffectsMechanicMask() & (1 << (*i)->GetMiscValue()))
                    drainPct += (*i)->GetAmount();
            }

            if (drainPct > 0)
            {
                int32 heal = CalculatePct(damage, drainPct);
                CastCustomSpell(this, 81488, &heal, NULL, NULL, true);
            }
        }
    }

    // Rage from Damage made (only from direct weapon damage)
    if (cleanDamage && damagetype == DIRECT_DAMAGE && this != victim && getClass() == CLASS_WARRIOR)
    {
    ...
    if (health <= damage && !victim->HasAuraType(SPELL_AURA_DEATH_ESCAPE))
    {
    ...
    else
    {
        ...
        victim->ModifyHealth(health <= damage ? 1 : -(int32)damage);
        ...
        switch (spellInfo->DmgClass)
        {
            ...
            case SPELL_DAMAGE_CLASS_MELEE:
            {
                ...
                if (crit)
                {
                    ...
                    critPctDamageMod += (victim->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_SCHOOL_CRIT_DMG_TAKEN, spellInfo->GetSchoolMask()) - 1.0f) * 100;

                    if (spellInfo->DmgClass == SPELL_DAMAGE_CLASS_RANGED)
                        critPctDamageMod += GetTotalAuraModifier(SPELL_AURA_MOD_CRIT_DAMAGE_RANGED);
                    else
                        critPctDamageMod += GetTotalAuraModifier(SPELL_AURA_MOD_CRIT_DAMAGE_MELEE);
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    switch (damageInfo->hitOutCome)
    {
        ...
        case MELEE_HIT_CRIT:
        {
            ...
            {
                mod += GetTotalAuraModifier(SPELL_AURA_MOD_CRIT_DAMAGE_RANGED);
                mod += damageInfo->target->GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKER_RANGED_CRIT_DAMAGE);
            }
            else
            {
                mod += GetTotalAuraModifier(SPELL_AURA_MOD_CRIT_DAMAGE_MELEE);
            ...
            mod += (damageInfo->target->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_SCHOOL_CRIT_DMG_TAKEN, damageInfo->damageSchoolMask) - 1.0f) * 100;
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo /*= NULL*/)
{
    ...
    // We're going to call functions which can modify content of the list during iteration over it's elements
    // Let's copy the list so we can prevent iterator invalidation
    AuraEffectList vPowerSchoolAbsorbCopy(victim->GetAuraEffectsByType(SPELL_AURA_POWER_SCHOOL_ABSORB));
    vPowerSchoolAbsorbCopy.sort(Trinity::AbsorbAuraOrderPred());

    // absorb without mana cost
    for (AuraEffectList::iterator itr = vPowerSchoolAbsorbCopy.begin(); (itr != vPowerSchoolAbsorbCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
    {
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst (const Unit* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
    ...
    int32    skillBonus  = 0/*4 * (attackerWeaponSkill - victimMaxSkillValueForLevel)*/;
    int32    sum = 0, tmp = 0;
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    int32 modHitChance = 84;
    if (GetTypeId() == TYPEID_UNIT)
    {
        if (ToCreature()->IsTrigger())
            modHitChance = 100;
        else
            modHitChance = 94;
    }
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
    ...
    data << uint32(1);                                      // count
    //data << uint32(aura->GetAuraType() == SPELL_AURA_PERIODIC_POWER_DAMAGE ? 
    //           SPELL_AURA_PERIODIC_DAMAGE : aura->GetAuraType());                    // auraId
    uint32 aura_id;
    switch (aura->GetAuraType())
    {
        case SPELL_AURA_PERIODIC_POWER_DAMAGE:
            aura_id = SPELL_AURA_PERIODIC_DAMAGE;
            break;
        case SPELL_AURA_PERIODIC_POWER_HEAL:
            aura_id = SPELL_AURA_PERIODIC_HEAL;
            break;
        default:
            aura_id = aura->GetAuraType();
            break;
    }
    data << aura_id;
...
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 56375:
                {
                    ...
                    target->RemoveAurasByType(SPELL_AURA_PERIODIC_POWER_LEECH);
                    return true;
                }
                // Glyph of Icy Veins
                case 56374:
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            switch (dummySpell->Id)
            {
                // Shadow bolt - only effect attacker who cast this aura
                case 686:
                {
                    if (victim == triggeredByAura->GetCaster())
                    {
                        int32 heal = CalculatePct(damage, triggeredByAura->GetAmount());
                        if (procSpell && procSpell->Id == 689) // Corrupt Life
                            heal *= 4;
                        victim->CastCustomSpell(victim, 80949, &heal, NULL, NULL, true);
                        return true;
                    }
                    break;
                }
                ...
                case 28176:
                {
                    if (victim)
                    {
                        int32 heal = std::min(victim->CountPctFromMaxHealth(triggeredByAura->GetAmount()), CountPctFromMaxHealth(20));
                        CastCustomSpell(this, 80953, &heal, NULL, NULL, true);
                        return true;
                    }
                    break;
                }
        ...
        {
            // 20925 - Devine Shield
            if (dummySpell->Id == 20925)
            {
                if (victim && victim->IsAlive())
                {
                    int32 damage = CalculatePct(GetShieldBlockValue(), triggeredByAura->GetAmount());
                    CastCustomSpell(victim, 80959, &damage, NULL, NULL, true);
                    return true;
                }
                break;
            }
            ...
            switch (dummySpell->Id)
            {
                ...
                case 80933:
                {
                    ...
                    // 2%(target mob) or 4%(target player) of maximum health
                    basepoints0 = GetTypeId() == TYPEID_PLAYER ? int32(victim->CountPctFromMaxHealth(4)) : int32(victim->CountPctFromMaxHealth(2));
                    victim->CastCustomSpell(victim, 80941, &basepoints0, 0, 0, true, 0, triggeredByAura);
                    return true;
                }
                // Seal of Wisdom
                case 80934:
                {
                    basepoints0 = victim->GetTypeId() == TYPEID_PLAYER ? 6 : 3;
                    CastCustomSpell(this, 80943, &basepoints0, 0, 0, true, 0, triggeredByAura);
                    return true;
                }
                // Judgement of Wisdom
                case 80935:
                {
                    ...
                    {
                        // 2% of base mana
                        basepoints0 = GetTypeId() == TYPEID_PLAYER ? 4 : 2;
                        victim->CastCustomSpell(victim, 80942, &basepoints0, NULL, NULL, true, 0, triggeredByAura);
                    }
                    return true;
                }
                // Judgement of Martyr
                case 80946:
                {
                    ...
                    {
                        basepoints0 = CalculatePct(damage, triggeredByAura->GetAmount());
                        CastCustomSpell(this, 80945, &basepoints0, NULL, NULL, true, 0, triggeredByAura);
                    }
                    return true;
                }
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 33757:
                {
                    ...
                    int32 extra_attack_power = CalculatePct(player->GetTotalAttackPowerValue(BASE_ATTACK), CalculateSpellDamage(victim, windfurySpellInfo, 1));

                    // Value gained from additional AP
                    basepoints0 = int32(extra_attack_power / 10.0f * GetAttackTime(attType) / 1000);
            ...
            if (dummySpell->SpellFamilyFlags[0] & 0x200000)
            {
                ...
                if (castItem->GetSlot() == EQUIPMENT_SLOT_MAINHAND && procFlag & PROC_FLAG_DONE_MAINHAND_ATTACK)
                {
                    float BaseWeaponSpeed = GetAttackTime(BASE_ATTACK) / 1000.0f;

                    // Value1: add the tooltip damage by swingspeed +  Value2: add spelldmg by swingspeed
                    basepoints0 = int32(BaseWeaponSpeed * 33.0f + 17.0f);
                    triggered_spell_id = 10444;
                }
...
bool Unit::HandleAuraProc(Unit* victim, uint32 damage, Aura* triggeredByAura, SpellInfo const* procSpell, uint32 /*procFlag*/, uint32 procEx, uint32 cooldown, bool * handled)
{
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
			...
			case 81200:
				if (triggeredByAura)
					triggeredByAura->Remove(AURA_REMOVE_BY_DEFAULT);
				break;
        ...
        if (Player* p = victim->ToPlayer())
        {
            if (Group* g = p->GetGroup())
            {
                for (GroupReference* itr = g->GetFirstMember(); itr != NULL; itr = itr->next())
                {
                    ...
                    int32 healPct = 0;
                    AuraEffectList const& a = groupGuy->GetAuraEffectsByType(SPELL_AURA_SHARE_HEAL_FROM_PARTY_NOT_STACK);
                    for (AuraEffectList::const_iterator i = a.begin(); i != a.end(); ++i)
                    {
                        SpellInfo const* s = (*i)->GetSpellInfo();
                        uint8 effIndex = (*i)->GetEffIndex();
                        if (s->Effects[effIndex].CalcRadius(groupGuy) <= victim->GetDistance2d(groupGuy))
                            healPct = std::max(healPct, (*i)->GetAmount());
                    }
                    if (healPct)
                    {
                        int32 heal = CalculatePct(addHealth, healPct);
                        groupGuy->CastCustomSpell(groupGuy, 81752, &heal, NULL, NULL, true);
                    }
                }
            }
        }
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack) const
{
    ...
    AuraEffectList const& mechanicDmgBenifit = owner->GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_DONE_FOR_MECHANIC);
    for (AuraEffectList::const_iterator i = mechanicDmgBenifit.begin(); i != mechanicDmgBenifit.end(); ++i)
    {
        if (spellProto->Mechanic == (*i)->GetMiscValue())
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        DoneAdvertisedBenefit += xCore->CalcXDiminish(GetStat(STAT_INTELLECT), 100);
            ...
            if (victim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    ...
                    case SPELLFAMILY_SHAMAN:
                        ...
                        {
                            if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_POWER_DAMAGE, SPELLFAMILY_SHAMAN, 0x10000000, 0, 0, GetGUID()))
            ...
            if (victim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    ...
                    case SPELLFAMILY_WARRIOR:
                       ...
                       // Overpower
                       if (spellProto->Id == 14895)
                       {
                           crit_chance += 50;
                           break;
                       }
                       // Slam
                       if (spellProto->Id == 50873)
                       {
                    ...
                    case SPELLFAMILY_HUNTER:
                        // 2973 - raptor strike
                        if (spellProto->SpellFamilyFlags[2] & 0x10000)
                        {
...
uint32 Unit::SpellCriticalDamageBonus(SpellInfo const* spellProto, uint32 damage, Unit* victim)
{
    ...
    crit_mod += GetTotalAuraModifier(SPELL_AURA_MOD_CRIT_DAMAGE_SPELL);

    if (victim)
    {
        crit_mod += GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_CRIT_PERCENT_VERSUS, victim->GetCreatureTypeMask());
        crit_mod += (victim->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_SCHOOL_CRIT_DMG_TAKEN, spellProto->GetSchoolMask()) - 1.0f) * 100;
    }
    ...
    */
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        switch (spellProto->Effects[i].ApplyAuraName)
        {
            ...
            case SPELL_AURA_PERIODIC_POWER_LEECH:
                DoneTotal = 0;
                break;
        }
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        advertisedBenefit += xCore->CalcXDiminish(GetStat(STAT_SPIRIT), 100);
...
uint32 Unit::MeleeDamageBonusDone(Unit* victim, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    AuraEffectList const& mDamageDonePctCreature = GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_DONE_CREATURE_PCT);
    for (AuraEffectList::const_iterator i = mDamageDonePctCreature.begin(); i != mDamageDonePctCreature.end(); ++i)
        if (creatureTypeMask & uint32((*i)->GetMiscValue()))
            AddPct(DoneTotalMod, (*i)->GetAmount());
...
float Unit::GetTotalStatValue(Stats stat) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            case STAT_STRENGTH:
                if (player->HasSkill(805))
                    value += player->GetSkillValue(805);
                break;
            case STAT_AGILITY:
                if (player->HasSkill(806))
                    value += player->GetSkillValue(806);
                break;
            case STAT_STAMINA:
                if (player->HasSkill(807))
                    value += player->GetSkillValue(807);
                break;
            case STAT_INTELLECT:
                if (player->HasSkill(808))
                    value += player->GetSkillValue(808);
                break;
            case STAT_SPIRIT:
                if (player->HasSkill(809))
                    value += player->GetSkillValue(809);
                break;
            default:
                break;
        }
...
uint32 Unit::GetCastingTimeForBonus(SpellInfo const* spellProto, DamageEffectType damagetype, uint32 CastingTime) const
{
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        switch (spellProto->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_POWER_DAMAGE:
                DirectDamage = true;
                break;
    ...
    for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
    {
        if (spellProto->Effects[j].Effect == SPELL_EFFECT_HEALTH_LEECH ||
            ...
            (spellProto->Effects[j].Effect == SPELL_EFFECT_APPLY_AURA && spellProto->Effects[j].ApplyAuraName == SPELL_AURA_PERIODIC_POWER_LEECH))
        {
            CastingTime /= 2;
            break;
        }
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (isRewardAllowed && player && player != victim)
    {
        ...
        if (creature && player->getLevel() - creature->getLevel() <= 5)
        {
            std::vector<Player*> reward_member_list;
            ...
            if (Group* group = player->GetGroup())
            {
                ...
                {
                    if (Player* member = itr->GetSource())
                        if (member->IsAtGroupRewardDistance(creature) && member->getLevel() - creature->getLevel() <= 5)
                            reward_member_list.push_back(member);
                }
            }
            else
                reward_member_list.push_back(player);

            if (!reward_member_list.size()) // shouldnt happen
                return;

            chance /= reward_member_list.size();

            switch (creature->GetCreatureTemplate()->rank)
            {
                case CREATURE_ELITE_NORMAL: // reward 1 player
                    reward_member_list[urand(0, reward_member_list.size() - 1)]->AttributeSkillUp(chance * 2.0f);
                    break;
                case CREATURE_ELITE_RARE: // reward 1 player
                    reward_member_list[urand(0, reward_member_list.size() - 1)]->AttributeSkillUp(chance * 30.0f); 
                    break;
                case CREATURE_ELITE_RAREELITE: // reward group
                    for (uint8 i = 0; i != reward_member_list.size(); i++)
                        reward_member_list[i]->AttributeSkillUp(chance * 60.0f);
                    break;
                case CREATURE_ELITE_ELITE: // reward 1 player
                    reward_member_list[urand(0, reward_member_list.size() - 1)]->AttributeSkillUp(chance * 10.0f);
                    break;
                case CREATURE_ELITE_WORLDBOSS:
                    for (uint8 i = 0; i != reward_member_list.size(); i++)
                        reward_member_list[i]->AttributeSkillUp(100.0f);
                    break;
                default:
                    break;
            }
        ...
        if (creature/* && xCore->IsEnabled(LEGACY_ENABLE_TYPE_CREATURE_LOOT, creature->GetCreatureTemplate()->Entry)*/)
        {
...
void Unit::RewardRage(uint32 damage, uint32 weaponSpeedHitFactor, bool attacker)
{
    ...
    ModifyPower(POWER_MANA, uint32(addRage));
}
