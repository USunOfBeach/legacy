...
float Player::GetHealthBonusFromStamina()
{
    ...
    */
    ...
    {
        case CLASS_WARLOCK:
            return GetStat(STAT_STAMINA) * 6;
        default:
            return GetStat(STAT_STAMINA) * 3;
    }
}
...
void Player::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    val2 = xCore->CalcXDiminish(GetStat(STAT_STRENGTH), 100.0f);
...
void Player::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, float& minDamage, float& maxDamage)
{
    ...
    float baseValue = GetModifierValue(unitMod, BASE_VALUE) + GetTotalAttackPowerValue(attType) / 10.0f * attackSpeedMod;
...
void Player::UpdateManaRegen()
{
    ...
    float power_regen = 0;//sqrt(Intellect) * OCTRegenMPPerSpirit();
