    ...
    if (HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING))
    {
        if (roll_chance_i(3) && GetTimeInnEnter() > 0)      // freeze update
        {
            ...
            if (time_inn >= 10)                             // freeze update
            {
				...
				float rest_bonus = GetRestBonus() + time_inn*((float)GetUInt32Value(PLAYER_NEXT_LEVEL_XP) / 72000)*bubble;
				float rest_xp_mod = 100.0f;
				AuraEffectList const& rest_bonus_auras = GetAuraEffectsByType(SPELL_AURA_MOD_REST_XP_GENERATE_RATE);
				for (AuraEffectList::const_iterator i = rest_bonus_auras.begin(); i != rest_bonus_auras.end(); ++i)
					rest_xp_mod += (*i)->GetAmount();
				ApplyPct(rest_bonus, rest_xp_mod);
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            switch (getClass())
            {
                ...
                case CLASS_HUNTER:
                    break;
                    // do mana calculations
                case CLASS_WARRIOR:
                    if (!IsInCombat() && !HasAuraType(SPELL_AURA_INTERRUPT_REGEN))
                    {
                        int32 cap = 0;
                        AuraEffectList const& a = GetAuraEffectsByType(SPELL_AURA_PREVENT_POWER_LOSS_AT_PCT_NOT_STACK);
                        for (AuraEffectList::const_iterator i = a.begin(); i != a.end(); ++i)
                            cap = std::max(cap, (*i)->GetAmount());

                        if (cap)
                        {
                            int32 newValue = curValue - CalculatePct(maxValue, cap);
                            if (!newValue)
                                return;
                            else if (newValue > 2)
                                addvalue -= 2;
...
void Player::RegenerateHealth()
{
    ...
    if (HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING)) // Regenerate 2% of health per tick (2 sec).
        addValue += maxValue * 0.02f;
...
uint32 Player::GetShieldBlockValue() const
{
    ...
    float value = (m_auraBaseMod[SHIELD_BLOCK_VALUE][FLAT_MOD] + xCore->CalcXDiminish(GetStat(STAT_STRENGTH), 100.0f) / 2) * m_auraBaseMod[SHIELD_BLOCK_VALUE][PCT_MOD];
...
{
	return xCore->CalcAgility2WeaponCrit(GetStat(STAT_AGILITY));
}

void Player::GetDodgeFromAgility(float &diminishing, float &nondiminishing)
{
	nondiminishing = xCore->CalcAgility2Dodge(GetStat(STAT_AGILITY));
    diminishing = 0.0f;
}
...
{
	return xCore->CalcSpirit2SpellCrit(GetStat(STAT_SPIRIT));
}

float Player::GetRatingMultiplier(CombatRating cr) const
{
	return xCore->CalcRatingMultiplier(cr);
}
...
void Player::UpdateSkillsForLevel()
{
    ...
    for (SkillStatusMap::iterator itr = mSkillStatus.begin(); itr != mSkillStatus.end(); ++itr)
    {
        ...
        if (max != 1)
        {
            ...
            if (alwaysMaxSkill)
            {
                SetUInt32Value(valueIndex, MAKE_SKILL_VALUE(isAttriSkill ? maxAttri : maxSkill, isAttriSkill ? maxAttri : maxSkill));
                if (itr->second.uState != SKILL_NEW)
                    itr->second.uState = SKILL_CHANGED;
            }
            else if (max != maxconfskill)                    /// update max skill value if current max skill not maximized
            {
                SetUInt32Value(valueIndex, MAKE_SKILL_VALUE(val, isAttriSkill ? maxAttri : maxSkill));
                if (itr->second.uState != SKILL_NEW)
                    itr->second.uState = SKILL_CHANGED;
            }
        }
...
uint16 Player::GetPureMaxSkillValue(uint32 skill) const
{
	...
	SkillRaceClassInfoEntry const* rcInfo = GetSkillRaceClassInfo(skill, getRace(), getClass());
	if (rcInfo)
	{
...
void Player::SendLoot(ObjectGuid guid, LootType loot_type)
{
    ...
    else
    {
        ...
        //if (loot_type == LOOT_PICKPOCKETING && creature->GetPickpocketRefillTimer() > time(NULL))
        //{
        //    ChatHandler(GetSession()).PSendSysMessage(30000);
        //    SendLootRelease(guid);
        //    return;
        //}
...
void Player::SendInitWorldStates(uint32 zoneid, uint32 areaid)
{
    ...
    switch (zoneid)
    {
        ...
        default:
            xEventMgr->FillZoneStatePocket(&data, zoneid);
...
//                            break;
                ...
                case ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET:
                    // nothing do..
                    break;
                case ITEM_ENCHANTMENT_TYPE_ACTIVE_SPELL:
                    if (enchant_spell_id)
                    {
                        if (apply)
                            LearnSpell(enchant_spell_id, false);
                        else
                            RemoveSpell(enchant_spell_id, false, false);
                    }
                    break;
                default:
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    else if (getLevel() - quest->GetQuestLevel() <= 5 || quest->GetQuestLevel() == -1)
    {
        switch (quest->GetXPId())
        {
            case 1:
            case 2:
                AttributeSkillUp(5.0f);
                break;
            case 3:
            case 4:
                AttributeSkillUp(15.0f);
                break;
            case 5:
            case 6:
                AttributeSkillUp(50.0f);
                break;
            case 7:
            case 8:
            case 9:
            case 10:
                AttributeSkillUp(100.0f);
                break;
            default:
                break;
        } 
...
QuestGiverStatus Player::GetQuestDialogStatus(Object* questgiver)
{
    ...
    for (QuestRelations::const_iterator i = qir.first; i != qir.second; ++i)
    {
        ...
        /*if (!xCore->IsEnabled(LEGACY_ENABLE_TYPE_QUEST, questId))
            continue;*/
        ...
        case SKILL_RANGE_LEVEL:
        {
            ...
            switch (skillId)
            {
                ...
                case 809:
                    ...
                    break;
                default:
                    maxValue = GetMaxSkillValueForLevel();
                    break;
            }
            ...
            if (rcInfo->Flags & SKILL_FLAG_ALWAYS_MAX_VALUE)
                ...
                skillValue = 1;/*std::min(std::max<uint16>({ 1, uint16((getLevel() - 1) * 5) }), maxValue);*/
...
void Player::HandleFall(MovementInfo const& movementInfo)
{
    ...
    if (z_diff >= 10.0f && !isDead() && !IsGameMaster() &&
        ...
        !HasAuraType(SPELL_AURA_FLY) && !IsImmunedToDamage(SPELL_SCHOOL_MASK_NORMAL))
    {
        ...
        float damageperc = 0.05f*(z_diff-safe_fall)-0.5f;
...
void Player::LearnTalent(uint32 talentId, uint32 talentRank)
{
    ...
    if (!xCore->IsTalentUnlocked(this, talentInfo))
        return;

    // Check if it requires another talent
    for (int8 i = 0; i != MAX_TALENT_DEPENDENCY; ++i)
    {
        if (talentInfo->DependsOn[i] > 0)
        {
            if (TalentEntry const* depTalentInfo = sTalentStore.LookupEntry(talentInfo->DependsOn[i]))
            {
                bool hasEnoughRank = false;
                for (uint8 rank = talentInfo->DependsOnRank[i]; rank < MAX_TALENT_RANK; rank++)
                {
    ...
    if (m_specsCount)
    {
        ...
        for (uint32 specIdx = 0; specIdx < m_specsCount; ++specIdx)
        {
            ...
            *data << uint8(talentIdCount);                  // [PH], talentIdCount
            ...
            uint32 talentTabId = GetTalentTabPage(getClass());
...
void Player::AttributeSkillUp(float chance /* = 100.0f*/, uint8 stat /* = 0 */, uint8 step /* = 0 */)
{
    if (frand(1.0f, 100.0f) > chance)
        return;

    // player must have all attribute skills to step up.
    // when at least 1 skill got maxvalue, return.
    if (!(HasSkill(805) && HasSkill(806) && HasSkill(807) && HasSkill(808) && HasSkill(809)))
        return;

    if (GetSkillValue(805) >= GetMaxSkillValue(805) ||
        GetSkillValue(806) >= GetMaxSkillValue(806) ||
        GetSkillValue(807) >= GetMaxSkillValue(807) ||
        GetSkillValue(808) >= GetMaxSkillValue(808) ||
        GetSkillValue(809) >= GetMaxSkillValue(809))
        return;

    uint8 attribute = stat;
    if (attribute == 0) // random
        attribute = urand(1, 5);
    if (step == 0)
        step = 1;
    ...
    {
        case 1: // strength
            UpdateSkill(805, step);
            CastSpell(this, 80962); // visual
            ChatHandler(GetSession()).PSendSysMessage(30001, step);
            break;
        case 2: // agility
            UpdateSkill(806, step);
            CastSpell(this, 80963); // visual
            ChatHandler(GetSession()).PSendSysMessage(30002, step);
            break;
        case 3: // stamina
            UpdateSkill(807, step);
            CastSpell(this, 80964); // visual
            ChatHandler(GetSession()).PSendSysMessage(30003, step);
            break;
        case 4: // intellect
            UpdateSkill(808, step);
            CastSpell(this, 80965); // visual
            ChatHandler(GetSession()).PSendSysMessage(30004, step);
            break;
        case 5: // spirit
            UpdateSkill(809, step);
            CastSpell(this, 80966); // visual
            ChatHandler(GetSession()).PSendSysMessage(30005, step);
            break;
        default:
            return;
    }
...
{
    Item* baseW = GetWeaponForAttack(BASE_ATTACK);
    Item* offW = GetWeaponForAttack(OFF_ATTACK);
    if ((att == BASE_ATTACK || att == OFF_ATTACK) && baseW && offW && baseW->GetTemplate()->Class == ITEM_CLASS_WEAPON && offW->GetTemplate()->Class == ITEM_CLASS_WEAPON)
    {
        int32 factor = 0;
        AuraEffectList const& a = GetAuraEffectsByType(SPELL_AURA_MOD_DUALWIELD_MELEE_HASTE);
        for (AuraEffectList::const_iterator i = a.begin(); i != a.end(); ++i)
            factor += (*i)->GetAmount();
...
void Player::UpdateAttackTime()
{
    // base
    if (Item* baseW = GetWeaponForAttack(BASE_ATTACK, true))
        SetAttackTime(BASE_ATTACK, baseW->GetTemplate()->Delay == 0 ? BASE_ATTACK_TIME : baseW->GetTemplate()->Delay);
    else
        SetAttackTime(BASE_ATTACK, BASE_ATTACK_TIME);

    // off
    if (Item* offW = GetWeaponForAttack(OFF_ATTACK, true))
        SetAttackTime(OFF_ATTACK, offW->GetTemplate()->Delay == 0 ? BASE_ATTACK_TIME : offW->GetTemplate()->Delay);
    else
        SetAttackTime(OFF_ATTACK, BASE_ATTACK_TIME);

    // ranged
    if (Item* rangedW = GetWeaponForAttack(RANGED_ATTACK, true))
        SetAttackTime(RANGED_ATTACK, rangedW->GetTemplate()->Delay == 0 ? BASE_ATTACK_TIME : rangedW->GetTemplate()->Delay);
    else
        SetAttackTime(RANGED_ATTACK, BASE_ATTACK_TIME);
}
