        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARRIOR:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1] & 0x200)
                {
                    ...
                    damage += CalculatePct(block_value, m_spellInfo->Effects[EFFECT_0].CalcValue());
                }
                // Devastate Slam
                if (m_spellInfo->SpellFamilyFlags[1] & 0x8000000)
                {
                    ...
                    if (!unitTarget->HasAura(58567, m_caster->GetGUID()))
                        m_caster->CastSpell(unitTarget, 58567, true);
                    else if (Aura* aur = unitTarget->GetAura(58567, m_caster->GetGUID()))
                    {
                        damage *= 1 + aur->GetStackAmount() * 0.1f;
                ...
                // Intercept
                if (m_spellInfo->SpellFamilyFlags[1] & 0x2000)
                {
                    damage = ApplyPct(damage, (100 - unitTarget->GetHealthPct()));
                    break;
                }
                break;
            }
            case SPELLFAMILY_WARLOCK:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1] & 0x40)
                {
                    ...
                    {
                        if (unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_POWER_DAMAGE, SPELLFAMILY_WARLOCK, 0x4, 0, 0))
                            damage += CalculatePct(damage, m_spellInfo->Effects[1].CalcValue(m_caster));
                    }
                }
                // 17962 - Conflagrate
                if (m_spellInfo->SpellFamilyFlags[1] & 0x800000)
                {
                    if (Aura* aura = unitTarget->GetAura(348))
                    {
                        uint32 pdamage = uint32(std::max(aura->GetEffect(0)->GetDamage(), 0));
                        pdamage = m_caster->SpellDamageBonusDone(unitTarget, aura->GetSpellInfo(), pdamage, DOT, aura->GetStackAmount());
                        pdamage = unitTarget->SpellDamageBonusTaken(m_caster, aura->GetSpellInfo(), pdamage, DOT, aura->GetStackAmount());
                        uint8 baseTotalTicks = uint8(m_caster->CalcSpellDuration(aura->GetSpellInfo()) / aura->GetSpellInfo()->Effects[EFFECT_0].Amplitude);
                        damage = int32(CalculatePct(pdamage * baseTotalTicks, m_spellInfo->Effects[0].CalcValue(m_caster)));
...
void Spell::EffectHeal(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
		...
		{
			case SPELLFAMILY_PALADIN:
				if (m_spellInfo->SpellFamilyFlags[0] & 0x80000000)
				{
					if (caster->HasAura(81212) && caster == unitTarget)
					{
						ApplyPct(addhealth, m_spellInfo->Effects[0].CalcValue());
						caster->AddAura(81213, unitTarget);
						break;
					}
				}
				break;
...
void Spell::EffectWeaponDmg(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARRIOR:
        {
            if (m_spellInfo->SpellFamilyFlags[0] & 0x40)
            {
                if (unitTarget->HasAuraWithMechanic(1 << MECHANIC_SNARE) ||
                    unitTarget->HasAuraWithMechanic(1 << MECHANIC_STUN) ||
                    unitTarget->HasAuraWithMechanic(1 << MECHANIC_ROOT))
            ...
            if (m_spellInfo->SpellFamilyFlags[1] & 0x40)
            {
                ...
                if (Aura* aur = unitTarget->GetAura(58567, m_caster->GetGUID()))
                {
                    ...
                    totalDamagePercentMod *= 1 + aur->GetStackAmount() * 0.1f;
                }
            }
    ...
    if (unitTarget && unitTarget->IsAlive())
    {
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARLOCK:
            {
                ...
                // 80955 - Soul Devour
                if (m_spellInfo->Id == 80955 && caster->GetTypeId() == TYPEID_PLAYER)
                {
                    if (Pet* pet = caster->ToPlayer()->GetPet())
                    {
                        int32 heal = m_originalCaster->SpellDamageBonusDone(unitTarget, m_spellInfo, (uint32)damage, SPELL_DIRECT_DAMAGE);
                        heal = unitTarget->SpellDamageBonusTaken(m_originalCaster, m_spellInfo, (uint32)heal, SPELL_DIRECT_DAMAGE);
                        heal /= 5; // 120% total heal of damage
                        unitTarget->CastCustomSpell(pet, 80956, &heal, NULL, NULL, true);
                        pet->CastCustomSpell(caster, 80957, &heal, NULL, NULL, true);
                    }
                    break;
                }
            }
            ...
            case SPELLFAMILY_ROGUE:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[0] & 0x20000)
                {
                    if (Player* player = m_caster->ToPlayer())
                    {
                        ...
                        {
                            // formula:
                            // 1cp:55% 2cp:100% 3cp:140% 4cp:175% 5cp:205%
                            uint32 ap = player->GetTotalAttackPowerValue(BASE_ATTACK);
                            switch (combo)
                            {
                                case 1:
                                    damage = ap * 0.55f;
                                ...
                                case 5:
                                    damage = ap * 2.05f;
                                    break;
                                default: // shouldn't happen, so just return for safety.
                                    return;
                            }
                        }
                    }
            ...
            case SPELLFAMILY_PALADIN:
            {
                if (m_spellInfo->Id == 80929) // Judgement of Command
                {
