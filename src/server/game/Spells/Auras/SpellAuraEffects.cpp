...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //317 SPELL_AURA_MOD_DAMAGE_DONE_CREATURE_PCT
    ...
    &AuraEffect::HandleDualWieldMeleeHaste,                       //324 SPELL_AURA_MOD_DUALWIELD_MELEE_HASTE
...
int32 AuraEffect::CalculateAmount(Unit* caster)
{
    ...
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_MANA_SHIELD:
            m_canBeRecalculated = false;
            break;
        case SPELL_AURA_POWER_SCHOOL_ABSORB:
        {
            // power school that affects absorb amount recorded in miscValueB.
            int32 pct = m_spellInfo->Effects[GetEffIndex()].CalcValue(caster);
            if (pct <= 0) break;
            float absorb = 0;
            switch (GetMiscValueB())
            {
                case 1: // Physical, by attack power or ranged attack power(max one).
                    absorb = std::max(caster->GetTotalAttackPowerValue(BASE_ATTACK), caster->GetTotalAttackPowerValue(RANGED_ATTACK));
                    break;
                case 2: // Holy, by holy spell damage power
                    absorb = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY);
                    break;
                case 4: // Fire
                    absorb = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FIRE);
                    break;
                case 8: // Nature
                    absorb = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_NATURE);
                    break;
                case 16: // Frost
                    absorb = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FROST);
                    break;
                case 32: // Shadow
                    absorb = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW);
                    break;
                case 64: // Arcane
                    absorb = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_ARCANE);
                    break;
                case -1: // Healing case
                    absorb = caster->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_ALL);
                    break;
                default:
                    break;
            }
            amount = CalculatePct(absorb, pct);
        }
            break;
        default:
            break;
...
void AuraEffect::CalculatePeriodic(Unit* caster, bool create, bool load)
{
    ...
    else // aura just created or reapplied
    {
        ...
        bool resetPeriodicTimer = create
            || ((GetAuraType() != SPELL_AURA_PERIODIC_DAMAGE) && (GetAuraType() != SPELL_AURA_PERIODIC_DAMAGE_PERCENT) && (GetAuraType() != SPELL_AURA_PERIODIC_POWER_DAMAGE));
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    else if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE_PERCENT)
    {
        switch (GetSpellInfo()->Id)
        {
            case 80879: // Nerve Berserk
                if (target->GetHealthPct() <= 5.0f)
                {
                    target->RemoveAurasDueToSpell(GetSpellInfo()->Id);
                    return;
                }
    ...
    else if (GetAuraType() == SPELL_AURA_PERIODIC_POWER_DAMAGE)
    {
        if (isAreaAura)
        {
            damage = CalculatePct(caster->SpellBaseDamageBonusDone(m_spellInfo->GetSchoolMask()), damage);
            damage = target->SpellDamageBonusTaken(caster, GetSpellInfo(), damage, DOT, GetBase()->GetStackAmount());
        }
        damage = target->SpellDamageBonusTaken(caster, GetSpellInfo(), damage, DOT, GetBase()->GetStackAmount());
        ...
        if (GetSpellInfo()->SpellFamilyName == SPELLFAMILY_WARLOCK && (GetSpellInfo()->SpellFamilyFlags[0] & 0x400) && GetSpellInfo()->SpellIconID == 544)
            damage += (m_tickNumber - 1) * damage / 10;
    }
    else
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    else if (GetAuraType() == SPELL_AURA_PERIODIC_POWER_HEAL)
    {
        if (isAreaAura)
...
void AuraEffect::HandlePeriodicEnergizeAuraTick(Unit* target, Unit* caster) const
{
    ...
    // don't regen when permanent aura target has full power - note: don't regen on full power case.
...
void AuraEffect::HandleDualWieldMeleeHaste(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    Unit* target = aurApp->GetTarget();
    ...
    target->ToPlayer()->UpdateAttackTime();
}
