    ...
    if (refresh)
    {
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_ROGUE:
                if (m_spellInfo->Id == 80912)
                {
                    if (Player* caster = GetCaster()->ToPlayer())
                    {
                        if (uint8 cp = caster->GetComboPoints())
                        {
                            if (Aura* proc_aura = GetCaster()->GetAura(80913))
                            {
                                GetEffect(0)->SetAmount(cp * proc_aura->GetEffect(1)->GetAmount());
                            }
                        }
                    }
                }
    ...
    if (apply)
    {
        switch (GetSpellInfo()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_ROGUE:
                ...
                if (GetSpellInfo()->Id == 80912) // Relentless Strike Energy regen
                {
                    if (Player* caster = GetCaster()->ToPlayer())
                    {
                        if (uint8 cp = caster->GetComboPoints())
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            ...
            case SPELL_AURA_PERIODIC_POWER_LEECH:
            {
                ...
                // Power damage only designed to be used by players.
                if (caster->GetTypeId() != TYPEID_PLAYER)
                    return;

                AuraEffect* aurEff = GetEffect(i);

                uint32 pct = std::max(0, m_spellInfo->Effects[i].CalcValue(caster));
                ...
                switch (GetSpellInfo()->DmgClass)
                {
                    ...
                    case SPELL_DAMAGE_CLASS_MAGIC:
                        power = caster->SpellBaseDamageBonusDone(m_spellInfo->GetSchoolMask());
                        break;
                ...
                uint32 damage = CalculatePct(power, pct);
            ...
            case SPELL_AURA_PERIODIC_POWER_HEAL:
            {
                ...
                switch (m_spellInfo->Effects[i].MiscValue)
                {
                    case 1: // Physical, by attack power or ranged attack power(max one).
                        power = std::max(caster->GetTotalAttackPowerValue(BASE_ATTACK), caster->GetTotalAttackPowerValue(RANGED_ATTACK));
                        break;
                    case 2: // Holy, by holy spell damage power
                        power = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY);
                        break;
                    case 4: // Fire
                        power = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FIRE);
                        break;
                    case 8: // Nature
                        power = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_NATURE);
                        break;
                    case 16: // Frost
                        power = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FROST);
                        break;
                    case 32: // Shadow
                        power = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW);
                        break;
                    case 64: // Arcane
                        power = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_ARCANE);
                        break;
                    case -1: // Healing case
                        power = caster->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_ALL);
                        break;
                    default: // no spell school for healing result so treat invalid misc value as heal case.
                        power = caster->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_ALL);
                        break;
                }
