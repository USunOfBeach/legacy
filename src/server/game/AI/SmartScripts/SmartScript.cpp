...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_SET_PLAYER_DATA:
        {
            uint32 set = e.action.legacyPlayerData.set;
            uint32 data = e.action.legacyPlayerData.data;
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsPlayer(*itr))
                    xCore->SetPlayerData((*itr)->ToPlayer(), set, data);
        ...
        case SMART_ACTION_LEARN_SPELL:
        {
            ...
            {
                if (IsPlayer(*itr))
                    (*itr)->ToPlayer()->LearnSpell(e.action.raw.param1, false);
            }
        ...
        case SMART_ACTION_UNLEARN_SPELL:
        {
            ...
            {
                if (IsPlayer(*itr))
                    (*itr)->ToPlayer()->RemoveSpell(e.action.raw.param1);
            }
