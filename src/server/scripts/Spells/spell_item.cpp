...
// itemset 945 6set - spell 81134
class legacy_spell_scount_itemset6_periodic : public SpellScriptLoader
{
public:
	legacy_spell_scount_itemset6_periodic() : SpellScriptLoader("legacy_spell_scount_itemset6_periodic") { }

	class legacy_spell_scount_itemset6_periodic_AuraScript : public AuraScript
	{
		PrepareAuraScript(legacy_spell_scount_itemset6_periodic_AuraScript);

		void OnPeriodic(AuraEffect const* aurEff)
		{
			PreventDefaultAction();
			if (GetCaster()->isMoving())
				GetCaster()->AddAura(81135, GetCaster());
			else
				GetCaster()->RemoveAurasDueToSpell(81135);
		}

		void Register() override
		{
			OnEffectPeriodic += AuraEffectPeriodicFn(legacy_spell_scount_itemset6_periodic_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new legacy_spell_scount_itemset6_periodic_AuraScript();
	}
};

// itemset 953 6 pieces - spell 81162
class legacy_spell_silver_exorcism_trigger : public SpellScriptLoader
{
public:
	legacy_spell_silver_exorcism_trigger() : SpellScriptLoader("legacy_spell_silver_exorcism_trigger") { }

	class legacy_spell_silver_exorcism_trigger_AuraScript : public AuraScript
	{
		PrepareAuraScript(legacy_spell_silver_exorcism_trigger_AuraScript);

		bool CheckProc(ProcEventInfo& eventInfo)
		{
			if (Unit* attacker = eventInfo.GetActionTarget())
			{
				if (Creature* c = attacker->ToCreature())
				{
					if (c->GetCreatureTemplate()->type == CREATURE_TYPE_UNDEAD)
						return true;
				}
			}
			return false;
		...
		void Register() override
		{
			DoCheckProc += AuraCheckProcFn(legacy_spell_silver_exorcism_trigger_AuraScript::CheckProc);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new legacy_spell_silver_exorcism_trigger_AuraScript();
	}
};

// itemset 954 4pieces spell 81164
class legacy_spell_81164_trigger : public SpellScriptLoader
{
public:
	legacy_spell_81164_trigger() : SpellScriptLoader("legacy_spell_81164_trigger") { }

	class legacy_spell_81164_trigger_AuraScript : public AuraScript
	{
		PrepareAuraScript(legacy_spell_81164_trigger_AuraScript);
		...
		void Register() override
		{
			DoCheckProc += AuraCheckProcFn(legacy_spell_81164_trigger_AuraScript::CheckProc);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new legacy_spell_81164_trigger_AuraScript();
	}
};

// itemset 957 4pieces spell 81175
class legacy_spell_81175_absorb : public SpellScriptLoader
{
public:
	legacy_spell_81175_absorb() : SpellScriptLoader("legacy_spell_81175_absorb") { }

	class legacy_spell_81175_absorb_AuraScript : public AuraScript
	{
		PrepareAuraScript(legacy_spell_81175_absorb_AuraScript);
		...
		void OnAbsorb(AuraEffect* aurEff, DamageInfo & dmgInfo, uint32 & absorbAmount)
		{
			...
			if (victim->GetHealth() - dmgInfo.GetDamage() <= 0)
			{
				if (urand(0, 1) == 0)
				{
					absorbAmount = dmgInfo.GetDamage();
					victim->SetHealth(victim->GetMaxHealth());
					victim->CastSpell(victim, 81176, true);
				}
			}
		}
		...
		void Register() override
		{
			DoEffectCalcAmount += AuraEffectCalcAmountFn(legacy_spell_81175_absorb_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
			OnEffectAbsorb += AuraEffectAbsorbFn(legacy_spell_81175_absorb_AuraScript::OnAbsorb, EFFECT_0);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new legacy_spell_81175_absorb_AuraScript();
	}
};

// itemset 959 - 9pieces - spell 81186
class legacy_spell_81186_trigger : public SpellScriptLoader
{
public:
	legacy_spell_81186_trigger() : SpellScriptLoader("legacy_spell_81186_trigger") { }

	class legacy_spell_81186_trigger_AuraScript : public AuraScript
	{
		PrepareAuraScript(legacy_spell_81186_trigger_AuraScript);
		...
		void OnPeriodic(AuraEffect const* aurEff)
		{
			...
			{
				if (caster->IsAlive() && caster->GetTypeId() == TYPEID_PLAYER)
				{
					if (Unit* selection = caster->ToPlayer()->GetSelectedUnit())
					{
						if (caster->IsHostileTo(selection))
						{
							caster->AddAura(81187, caster);
							return;
						}
					}
				}
				caster->RemoveAurasDueToSpell(81187);
			}
		}

		void Register() override
		{
			OnEffectPeriodic += AuraEffectPeriodicFn(legacy_spell_81186_trigger_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new legacy_spell_81186_trigger_AuraScript();
	}
};

// itemset 960 - 6pieces - spell 81190
class legacy_spell_81190_trigger : public SpellScriptLoader
{
public:
	legacy_spell_81190_trigger() : SpellScriptLoader("legacy_spell_81190_trigger") { }

	class legacy_spell_81190_trigger_AuraScript : public AuraScript
	{
		PrepareAuraScript(legacy_spell_81190_trigger_AuraScript);
		...
		void AfterTriggered(AuraEffect const* /*aurEff*/, ProcEventInfo& procInfo)
		{
			PreventDefaultAction();
			int32 dmg = procInfo.GetDamageInfo()->GetDamage();
			int32 heal = CalculatePct(dmg, GetSpellInfo()->Effects[EFFECT_0].CalcValue());
			GetCaster()->CastCustomSpell(GetCaster(), 81191, &heal, NULL, NULL, true);
		}

		void Register() override
		{
			DoCheckProc += AuraCheckProcFn(legacy_spell_81190_trigger_AuraScript::CheckProc);
			AfterEffectProc += AuraEffectProcFn(legacy_spell_81190_trigger_AuraScript::AfterTriggered, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new legacy_spell_81190_trigger_AuraScript();
	}
};

// itemset 960 - 8pieces - spell 81192
class legacy_spell_81192_trigger : public SpellScriptLoader
{
public:
	legacy_spell_81192_trigger() : SpellScriptLoader("legacy_spell_81192_trigger") { }

	class legacy_spell_81192_trigger_AuraScript : public AuraScript
	{
		PrepareAuraScript(legacy_spell_81192_trigger_AuraScript);
		...
		void OnTriggered(AuraEffect const* /*aurEff*/, ProcEventInfo& procInfo)
		{
			PreventDefaultAction();
			Unit* caster = procInfo.GetProcTarget();
			if (Player* player = caster->ToPlayer())
			{
				if (!player->HasSpellCooldown(81192))
				{
					caster->RemoveAurasByType(SPELL_AURA_MOD_STUN);
					caster->CastSpell(caster, 81192);
					player->AddSpellCooldown(81192, 0, time(NULL) + 40);
				}
			}
		}
		...
		void Register() override
		{
			DoCheckProc += AuraCheckProcFn(legacy_spell_81192_trigger_AuraScript::CheckProc);
			OnEffectProc += AuraEffectProcFn(legacy_spell_81192_trigger_AuraScript::OnTriggered, EFFECT_0, SPELL_AURA_DUMMY);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new legacy_spell_81192_trigger_AuraScript();
	}
};

// itemset 961 - 6 pieces - spell 81199
class legacy_spell_81199_trigger : public SpellScriptLoader
{
public:
	legacy_spell_81199_trigger() : SpellScriptLoader("legacy_spell_81199_trigger") { }

	class legacy_spell_81199_trigger_AuraScript : public AuraScript
	{
		PrepareAuraScript(legacy_spell_81199_trigger_AuraScript);

		void OnPeriodicTrigger(AuraEffect const* /*aurEff*/)
		{
			if (!GetCaster()->IsInCombat() && GetCaster()->IsAlive())
				GetCaster()->AddAura(81200, GetCaster());
		}

		void Register() override
		{
			OnEffectPeriodic += AuraEffectPeriodicFn(legacy_spell_81199_trigger_AuraScript::OnPeriodicTrigger, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new legacy_spell_81199_trigger_AuraScript();
	}
};
...
void AddSC_item_spell_scripts()
{
	...
	new legacy_spell_scount_itemset6_periodic();
	new legacy_spell_silver_exorcism_trigger();
	new legacy_spell_81164_trigger();
	new legacy_spell_81175_absorb();
	new legacy_spell_81186_trigger();
	new legacy_spell_81190_trigger();
	new legacy_spell_81192_trigger();
	new legacy_spell_81199_trigger();
}
