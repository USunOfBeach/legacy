...
class spell_sha_chain_heal : public SpellScriptLoader
{
    public:
        ...
        private:
            ...
            void Register() override
            {
                OnEffectHitTarget += SpellEffectFn(spell_sha_chain_heal_SpellScript::HandleHeal, EFFECT_0, SPELL_EFFECT_POWER_HEAL);
            }
...
class spell_sha_healing_stream_totem : public SpellScriptLoader
{
    public:
        ...
        class spell_sha_healing_stream_totem_SpellScript : public SpellScript
        {
            ...
            bool Validate(SpellInfo const* /*spellInfo*/) override
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_SHAMAN_TOTEM_HEALING_STREAM_HEAL))
                    return false;
                return true;
            }
                ...
                if (Unit* target = GetHitUnit())
                {
                    ...
                    if (Unit* caster = GetCaster())
                    {
                        ...
                        if (Unit* owner = caster->GetOwner())
                            damage = std::max(damage, CalculatePct(owner->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_ALL), GetSpellInfo()->Effects[1].CalcValue()));
