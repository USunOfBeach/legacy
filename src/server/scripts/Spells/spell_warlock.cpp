    ...
    public:
        ...
        class spell_warl_life_tap_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* target = GetHitUnit())
                {
                    int32 damage = CalculatePct(caster->GetMaxHealth(), GetSpellInfo()->Effects[0].CalcValue(caster));
                    int32 mana = CalculatePct(caster->GetMaxPower(POWER_MANA), GetSpellInfo()->Effects[1].CalcValue(caster));

                    caster->ModifyHealth(-damage);
                    caster->CastCustomSpell(caster, SPELL_WARLOCK_LIFE_TAP_ENERGIZE, &mana, NULL, NULL, false);
                }
            }

            SpellCastResult CheckCast()
            {
                if (int32(GetCaster()->GetHealthPct() > GetSpellInfo()->Effects[0].CalcValue(GetCaster())))
                    return SPELL_CAST_OK;
        ...
        class spell_warl_unstable_affliction_AuraScript : public AuraScript
        {
            ...
            void HandleDispel(DispelInfo* dispelInfo)
            {
                if (Unit* caster = GetCaster())
                    ...
                    {
                        uint8 max_ticks = uint8(caster->CalcSpellDuration(GetSpellInfo()) / GetSpellInfo()->Effects[EFFECT_0].Amplitude);
                        uint8 remain_ticks = GetDuration() / GetSpellInfo()->Effects[EFFECT_0].Amplitude;
                        int32 damage = CalculatePct(CalculatePct(caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW), GetSpellInfo()->Effects[2].CalcValue(caster)), GetSpellInfo()->Effects[1].CalcValue(caster) * (max_ticks - remain_ticks));
